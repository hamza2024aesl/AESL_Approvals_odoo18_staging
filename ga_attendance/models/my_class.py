...
def calc_standard_hrs(working_hours):
    ...
    if working_hours.uom_id.factor_inv:
        hours = str(working_hours.uom_id.factor_inv).split(".")
...
class HrAttendenceInherit(models.Model):
    ...
    def updateRecord(self):
        ...
        if self.new_check_in or self.new_check_out:
            a = self.new_check_in

            a = datetime.datetime.strptime(str(a), "%Y-%m-%d %H:%M:%S").date()

            check_in = str(self.new_check_in)
            check_out = str(self.new_check_out)
            compensatory = 'no'
            ...
            #             holidays = self.env['holidays.schedule'].search([])
            #
            #             if holidays:
            #                 holidays_id = holidays[0].holidays_id
            #
            #                 for holiday in holidays_id:
            #                     if str(a) == holiday.date:
            #                         h ='off_day'
            #
            #
            ...
            if emp_contract:
                working_hours = emp_contract.resource_calendar_id

                in_policy = working_hours.in_policy_id
                out_policy = working_hours.out_policy_id
                working_time = working_hours.attendance_ids
                compensatory_hrs = working_hours.compensatory_hours
                ...
                if check_in and check_out:
                    ...
                    # standard_hours = working_hours.uom_id.factor_inv
                    ...
                    if working_day == 'yes':
                        latetimein = str(end.time()).split(":")
                        actual_minutes = calc_minutes(latetimein)
                        diff = (sch_out - actual_minutes)

                        lt = check_ok_status(out_policy)

                        early_time_out = diff - int(lt)

                        out_time_in_hours = abs(calc_working_hours(early_time_out))

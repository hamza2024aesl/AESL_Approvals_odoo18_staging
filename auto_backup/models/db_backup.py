...
class DbBackup(models.Model):
    ...
    def schedule_backup(self):
        ...
        for rec in conf_ids:
            ...
            bkp_file = '%s_%s.%s' % (time.strftime('%Y_%m_%d_%H_%M_%S'), self._cr.dbname, rec.backup_type)
            file_path = os.path.join(rec.folder, bkp_file)
            fp = open(file_path, 'wb')
            try:
                # try to backup database and write it away
                fp = open(file_path, 'wb')
                self._take_dump(self._cr.dbname, fp, 'db.backup', rec.backup_type)
                fp.close()
            except Exception as error:
                _logger.debug(
                    "Couldn't backup database %s. Bad database administrator password for server running at "
                    "http://%s:%s" % (self._cr.dbname, rec.host, rec.port))
            ...
            if rec.autoremove:
                ...
                for f in os.listdir(directory):
                    ...
                    if self._cr.dbname in fullpath:

...
class resourceCalendar(models.Model):
    ...
    shift = fields.Selection([('day', 'Day'), ('night', 'Night')], string="Shift", required=False, default='day')
    ...
    @api.constrains('in_policy_id')
    def _check_unique_checkmark(self):
        adjust_time_check = self.in_policy_id.filtered(lambda r: r.adjust_time)
        if len(adjust_time_check) > 1:
            raise ValidationError("Only one policy can have 'adjust_time' set to True.")
...
class workingScheduleIn(models.Model):
    ...
    status = fields.Selection(
        [('0', 'OK'), ('1', 'Late-In'), ('2', 'Quarter-Day'), ('3', 'Half-Day'), ('4', 'Tri-Quarter'),
         ('5', 'Full-Day')])
    adjust_time = fields.Boolean("Adjust Time")

...
class my_change_request(models.Model):
	...
	_description = 'My Change Request'
	...
	manager = fields.Many2one('hr.employee', compute='get_my_manager',default=False)
	...
	def generate_request(self):
		if self.new_check_out <= self.new_check_in:
			raise UserError(('Check In time must preceed the Check Out time!'))
	...
	def copy(self):
		raise UserError('You cannot duplicate a change request, because it is unique for every attendance.')
	...
	def validate_request_manager(self):
		self.state = 'second'

	def validate_request_hr(self):
		if not(self.new_check_in or self.new_check_out):
			raise UserError(('You must enter either \"New Check In\" time or \"New Check Out\" time.'))
		...
		if self.new_check_in:
			self.attendance_id.check_in = self.new_check_in		
   
		if self.new_check_out:
			self.attendance_id.check_out = self.new_check_out	

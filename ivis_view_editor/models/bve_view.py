...
class BveView(models.Model):
    ...
    def _create_bve_view(self):
        ...
        list_view = View.create(
            {
                ...
                "arch": """<?xml version="1.0"?>
                    ...
                    """.format(
                    "".join(self._create_list_view_arch())
                ),
            }
        )
        ...
        self.write(
            {"action_id": action.id, "view_id": list_view.id, "state": "created"}
        )
    ...
    def _compute_sql_query(self):
        for bve_view in self:
            tables_map = {}
            jsonbs_list = ['standard_price'] # To account for jsonb database fields
            ...
            for line in bve_view.field_ids:
                ...
                if not any(jsonb in as_name for jsonb in jsonbs_list):
                    select_str += ",\n {}.{} AS {}".format(table, select, as_name)
                else:
                    select_str += f",\nCOALESCE({select}s.{select}_reference, 0) AS {as_name}"
                    lateral_str += f"\nLEFT JOIN LATERAL (SELECT CAST({table}.{select}->>jsonb_object_keys({table}.{select}) AS FLOAT) AS {select}_reference) AS {select}s ON true"

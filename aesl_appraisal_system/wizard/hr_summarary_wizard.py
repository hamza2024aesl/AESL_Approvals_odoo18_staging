...
class HrSummaryWizard(models.TransientModel):
    ...
    all_dept = fields.Boolean('All Region', deafult=False)
                ...
                for emp_line in emp.sudo().recomm_increment_lines_id:
                    ...
                    if emp_line.increment_raise_by.name not in mngr_nmae:
                        ...
                        grouped_records[group_key].append({
                            ...
                            'recomm_salary': (emp_line.increment_raise_amount + emp.gross_salary) if emp_line.increment_raise_amount else 0,
        ...
        for dept in self.department_ids:
            ...
            for emp in appraisals:
                if emp.appraisal_approver_id:
                    # region = emp.location_id.name
                    manager_name = emp.appraisal_approver_id.name
                    filter_line = emp.recomm_increment_lines_id.filtered(
                        lambda line: line.increment_raise_by.id == emp.appraisal_approver_id.id)

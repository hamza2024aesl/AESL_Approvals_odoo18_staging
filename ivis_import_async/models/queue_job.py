...
from ..controllers.main import PG_RETRY
...
class QueueJob(models.Model):
    ...
    def open_graph_jobs(self):
        ...
        action = self.env["ir.actions.act_window"]._for_xml_id(
            "ivis_import_async.action_queue_job"
        )
        action.update(
            {
    ...
    def _message_post_on_failure(self):
        ...
        for record in self:
            ...
            if msg:
                record.message_post(body=msg, subtype_xmlid="ivis_import_async.mt_job_failed")

    def _subscribe_users_domain(self):
        """Subscribe all users having the 'Queue Job Manager' group"""
        group = self.env.ref("ivis_import_async.group_queue_job_manager")
        if not group:
            return None

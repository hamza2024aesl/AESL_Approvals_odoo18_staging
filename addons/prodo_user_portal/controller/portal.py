...
class AppraisalPortal(CustomerPortal):
    ...
    def portal_appraisal_bulk_save(self, **post):
        ...
        user = request.env.user
        for appraisal in appraisals:
            last_increment_line = appraisal.recomm_increment_lines_id.sudo().search([], limit=1,
...
class TravelRequestPortal(CustomerPortal):

    @http.route(["/my/travel", "/my/travel/page/<int:page>"], type="http", auth="user", website=True)
    def portal_travel_list(self, page=1, **kw):
        user = request.env.user
        ...
        requests = request_obj.sudo().search(domain, limit=10, offset=pager_detail['offset'])
    ...
    def portal_travel_new(self, **kw):
        user = request.env.user
        employee = user.employee_id or request.env['hr.employee'].sudo().search([
            '|', '|',
            ('user_id', '=', user.id),
            ('work_contact_id', '=', user.partner_id.id),
            ('user_id.partner_id', '=', user.partner_id.id)
        ], limit=1)
        ...
        vals = {
            ...
            'employee_department_id': employee.department_id.name or '',
            'employee_work_location_id': employee.work_location_id.name or '',
            'employee_job_id': employee.job_id.name or '',
        }
    ...
    def portal_travel_save(self, **post):
        user = request.env.user
        employee = user.employee_id or request.env['hr.employee'].sudo().search([
            '|', '|',
            ('user_id', '=', user.id),
        ...
        vals = {
            'name': f"Travel Request - {employee.name if employee else user.name}",
        ...
        # In approval_ext, employee_id is a field on approval.request
        if employee:
    ...
    def portal_travel_view(self, request_id, **kw):
        request_rec = request.env['approval.request'].sudo().browse(request_id)
        if not request_rec.exists() or request_rec.request_owner_id.id != request.env.user.id:
            return request.redirect("/my/travel")

        user = request.env.user
        employee = request_rec.employee_id or user.employee_id or request.env['hr.employee'].sudo().search([
            '|', '|',
            ('user_id', '=', user.id),
            ('work_contact_id', '=', user.partner_id.id),
            ('user_id.partner_id', '=', user.partner_id.id)
        ...
        # Editable if in draft or refused state
        is_editable = request_rec.request_status in ['new', 'refused'] and request_rec.request_owner_id.id == user.id

        # Check if current user is a pending approver
        is_approver = any(a.user_id.id == user.id and a.status == 'pending' for a in request_rec.approver_ids)
    ...
    def portal_travel_approve(self, request_id, **kw):
        request_rec = request.env['approval.request'].sudo().browse(request_id)
        if not request_rec.exists():
            return request.redirect("/my/travel")
        
        # Check if user is an approver for this request
        approver = request_rec.approver_ids.filtered(lambda a: a.user_id.id == request.env.user.id and a.status == 'pending')
        if approver:
            request_rec.sudo().action_approve(approver=approver)
    ...
    def portal_travel_refuse(self, request_id, **kw):
        request_rec = request.env['approval.request'].sudo().browse(request_id)
        if not request_rec.exists():
            return request.redirect("/my/travel")
        ...
        approver = request_rec.approver_ids.filtered(lambda a: a.user_id.id == request.env.user.id and a.status == 'pending')
        if approver:
            request_rec.sudo().action_refuse(approver=approver)
    ...
    def portal_travel_update(self, **post):
        ...
        if request_rec.request_status not in ['new', 'refused']:
        ...
        request_rec.write(vals)
        
        # If it was refused or still in draft, we want to re-submit it
        if request_rec.request_status == 'refused':
            # Manually reset status to draft ('new') so action_confirm() can process it again
            request_rec.sudo().write({'request_status': 'new'})
            
        request_rec.action_confirm()
